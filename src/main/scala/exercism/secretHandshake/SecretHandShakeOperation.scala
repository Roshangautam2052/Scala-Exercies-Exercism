...
class SecretHandShakeOperation {
  ...
  private def mapBinaryEquivalentToActions(binaryNumber: List[Int]): Either[String, List[String]] = {
    ...
    def mapInnerActions(acc: List[String], currentList: List[Int], iterator: Int): Either[String, List[String]] = {
      ...
      if (currentList.isEmpty || iteration > 5) {
      ...
      currentValue match {
        ...
        case 1 => iteration match {
          case 1 => mapInnerActions(acc :+ "wink", remainingList, iteration)
          case 2 => mapInnerActions(acc :+ "double blink", remainingList, iteration)
          case 3 => mapInnerActions(acc :+ "close your eyes", remainingList, iteration)
          case 4 => mapInnerActions(acc :+ "jump", remainingList, iteration)
